...
const createOrder = async () => {
  ...
  if (!regex.test(clientName)) {
    sendMessage(chalk.redBright('Você precisa informar um nome válido! \n  Ex: "Ana Paula", "Paula"'));
    return;
  }
  ...
  if (selectItems.length <= 0) {
    sendMessage(chalk.redBright('Você precisa selecionar itens para incluir no seu pedido!'));
    return;
  }
...
const changeOrder = async () => {
  ...
  const listedItems = await checkbox({
    message: 'Selecione o pedido para marcar como concluído utilizando o espaço!',
    choices: getOrders
  });
...
const showStatus = async status => {
  ...
  // Formatando a mensagem para aparecer na Tabela
  const statusMap = orderStatus.map(item => `#${item.id} ${item.client} - ${item.status}`);
